...
const ProductSinglePage: React.FC = () => {
  ...
  const product = useSelector((state: RootState) => getProductSingle(state)[0]);
  const productSingleStatus = useSelector((state: RootState) => getSingleProductStatus(state));
  const [quantity, setQuantity] = useState<number>(1);
  const cartMessageStatus = useSelector((state: RootState) => getCartMessageStatus(state));
  ...
  let discountedPrice = (product?.price || 0) - (product?.price || 0) * (product?.discountPercentage || 0) / 100;

  const increaseQty = (): void => {
    setQuantity((prevQty) => {
      let tempQty = prevQty + 1;
      if (tempQty > (product?.stock || 0)) tempQty = product?.stock || 0;
      return tempQty;
    });
  };
